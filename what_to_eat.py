...
def choose_combo(dishes, history=None, window=30):
    """选一套：主菜 + 汤 + 主食（30 天不重复）"""
    history = history or []
    recent = {h["dish"] for h in history[-window:]}
...
def choose_kid_combo(dishes, history=None, window=30):
    """选一套儿童餐"""
    kid_dishes = filter_kid_friendly(dishes)
    return choose_combo(kid_dishes, history, window)
...
def choose_three_meals(dishes, history=None, window=7):
